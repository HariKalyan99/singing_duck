...
export const clearErrors = mutation({
  ...
  handler: async (ctx) => {
    ...
    const replayTransactions = await ctx.db
    ...
    return {
      success: true,
      deletedCount: errors.length,
      deletedReplayTransactions: replayTransactions.length,
    };
  },
});
...
export const updateErrorCodeSnippet = mutation({
  args: {
    errorId: v.id("errors"),
    codeSnippet: v.array(
      v.object({
